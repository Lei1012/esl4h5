<template>
	<view class="wrap">
		<view class="u-tabs-box">
			<u-tabs-swiper activeColor="#0aa0a8" ref="tabs" :list="list" :current="current" @change="change"
		...
		<swiper class="swiper-box" :current="swiperCurrent" @transition="transition" @animationfinish="animationfinish">
			<swiper-item class="swiper-item">
				...
				<scroll-view scroll-y class="scroll-container" v-if="approvedStatus"
					@scrolltolower="reachBottomApproved()">
					<view class=" list-item" v-for="(item,index) in jobsListApproved" :key="index">
						<view class="edit" @click="turnEditJob(item.id)">
							<image src="/pagesA/static/jobs/edit-job.png" mode="aspectFill"></image>
						</view>
						<view class="list-item-left">
							<image @click="turnBusinessProfile(item.user_id)"
						...
						<view class="list-item-right">
							...
							<view class="business-location">
								...
								<view class="job-location"
									v-if="item.city != 0 && (languageValue=='en-US' || !languageValue)  ">
									{{item.citys.Pinyin}}
								</view>
						...
						<view class="xll-view-openclose">
							<view class="xll-views">
								<uni-icons type="eye"></uni-icons><text>{{item.views}}</text>
							</view>
							...
							<view class="xll-refresh">
								<button type="default" @click="refreshJob(item.id)">
			...
			<swiper-item class="swiper-item">
				...
				<scroll-view scroll-y class="scroll-container" v-if="underReviewStatus"
					@scrolltolower="reachBottomUnderReview()">
			...
			<swiper-item class="swiper-item">
				...
				<scroll-view scroll-y class="scroll-container" v-if="rejectedStatus"
					@scrolltolower="reachBottomRejected()">
	...
	export default {
		...
		methods: {
			...
			refreshJob(jobId) {
				...
				jobs.refreshJobs(data).then(res => {
					if (res.code == 200) {
						uni.showToast({
							title: '',
							icon: 'success'
			...
			turnBusinessProfile(id) {
				...
				// #ifndef H5
				uni.navigateTo({
					url: '/pagesB/me/business/share?id=' + id + '&type=job'
				})
				// #endif
