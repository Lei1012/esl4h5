	...
	<view class="uni-flex uni-column home-bg">
		<view class="flex-item home-picture" :style="{backgroundImage:'url('+backgroundPictureSrc+')'}">
			<canvas canvas-id="canvasArcbar1" id="canvasArcbar1" class="charts3">
				<image
					:src="educatorInfo.profile_photo != '' ? educatorInfo.profile_photo :  'https://oss.esl-passport.cn/educator.png' "
					mode="aspectFill" @click="turnEditProfilePhoto(1,educatorInfo.profile_photo)"></image>
				<view
		...
		<view class="flex-item home-info">
			<uni-segmented-control class="segmented-control-bg" :current="current" :values="items"
				@clickItem="onClickItem" style-type="text" active-color="#333333"></uni-segmented-control>
			<view class="profile-content">
					...
					<view class="basic-info">
						<view class="basic-info-t">
							...
							<view class="edit-icon" @click="turnEditProfileBasic">
								<image src="/pagesB/static/esl/edit.png" mode="aspectFill"></image>
							</view>
						</view>
						<view class="basic-info-b">
							<view class="basic-info-item" style="width: 100%;"
								v-if="educatorInfo.first_name!='' || educatorInfo.last_name != '' ">
							...
							<view class="basic-info-item"
								v-if="basicUserInfo.birthday!='' && basicUserInfo.birthday != '0000-00-00' ">
							...
							<view class="basic-info-item"
								v-if="basicUserInfo.sex==1 || basicUserInfo.sex==2 || basicUserInfo.sex ==3">
						...
						<view class="credentials-b">
							<view class="languages">
								<view class="languages-top">
									<view class="languages-title">{{i18n.profilelanguages}}<text
											class="error-star">*</text></view>
								...
								<view class="languages-container">
									<view class="languages-item" v-for="(language,i) in languagesList" :key="i">
										...
										<u-rate :disabled="true" custom-prefix="custom-icon" active-icon="xll-yuan"
											inactive-icon="circleo" active-color="#b1c452"
											v-model="language.object_score" :count="language.object_score"></u-rate>
									</view>
								</view>
							</view>
							<view class="certifications">
								<view class="certifications-t">
									<view class="certifications-title">{{i18n.profilecertifications}}<text
											class="error-star">*</text></view>
									<view class="edit-icon" @click="turnIndexList(7)"
										v-if="canEditCertifications === false">
										<image src="/pagesB/static/esl/edit.png" mode="aspectFill"></image>
									</view>
									<view class="edit-icon" @click="certificationsConfirm" v-if="canEditCertifications">
										<image src="/pagesB/static/esl/confirm-icon.png" mode="aspectFill"></image>
									</view>
								</view>
								<!-- certifications 展示 -->
								<view class="certifications-b" v-if="canEditCertifications === false">
									<view class="certifications-item" v-for="(cer,i) in certificationsList" :key="i">
										<block v-if="languageValue=='en-US'">{{cer.object_en}}</block>
										<block v-if="languageValue=='zh-CN'">{{cer.object_cn}}</block>
									</view>
								</view>
								...
								<view class="jobs-tags-container" v-if="canEditCertifications">
									<view class="jobs-tags">
										<block>
											<view class="jobs-tags-item"
												...
												v-for="(item,index) in editCertificationsList" :key="item.id"
									...
									<view class="jobs-tags-item" v-if="addCertificationsStatus==false"
										@click="addCertificationsStatus=true">Add+</view>
									<view class="jobs-tags-add">
										<view class="jobs-tags-item-add" v-if="addCertificationsStatus">
											<input type="text" v-model="ownCertificationsValue"
												placeholder="Add certifications">
											<view class="jobs-tags-item-add-button">
												<button type="default" v-if="ownCertificationsValue.length>0"
													@click="addOwnCertifications">Confirm</button>
												<button type="default" v-if="ownCertificationsValue.length==0"
													@click="addCertificationsStatus=false">Cancel</button>
											</view>
										</view>
									</view>
								</view>
							...
							<view class="education">
								<view class="education-t">
									<view class="education-title">{{i18n.profileeducation}}<text
											class="error-star">*</text></view>
									<view class="add-icon" @click="turnEducation">
										<image src="/pagesB/static/esl/add.png" mode="aspectFill"></image>
									</view>
								</view>
									...
									<view v-for="(education,i) in educationInfo" :key="i">
										<view class="education-item" v-if="i<=educationNum">
											...
											<view class="education-item-3">
												...
												<view class="education-date">
													{{education.start_time | date('mm/yyyy')}}-{{education.end_time | date('mm/yyyy')}}
												</view>
											</view>
											<view style="border: 1rpx solid #E1E1E1;"></view>
										</view>
									</view>
									<view class="show-more" v-if="showMoreEducationStatus " @click="showMoreEducation">
										Show more</view>
								</view>
							</view>
						</view>
					...
					<view class="experience">
						...
						<view class="countries-travel">
							<view class="countries-travel-t">
								<view class="countries-travel-title countries-travel-fontsize">
									{{i18n.educatorprofileteachexp}}</view>
							...
							<view class="jobs-tags-container" v-if="canEditTeachExp">
								<view class="jobs-tags">
									<view class="jobs-tags-item"
										...
										v-for="(item,index) in editTeachExpList" :key="item.id"
								...
								<view class="jobs-tags-item" v-if="addTeachExpStatus==false"
									@click="addTeachExpStatus=true">Add+</view>
								<view class="jobs-tags-add">
									<view class="jobs-tags-item-add" v-if="addTeachExpStatus">
										<input type="text" v-model="ownTeachExpValue"
											placeholder="Add teaching experience">
										<view class="jobs-tags-item-add-button">
											<button type="default" v-if="ownTeachExpValue.length>0"
												@click="addOwnTeachExp">Confirm</button>
											<button type="default" v-if="ownTeachExpValue.length==0"
												@click="addTeachExpStatus=false">Cancel</button>
										</view>
									</view>
								</view>
							</view>
						...
						<view class="countries-travel">
							<view class="countries-travel-t">
								<view class="countries-travel-title countries-travel-fontsize">
									{{i18n.profilecountriestraveled}}</view>
								<view class="edit-icon" @click="turnIndexList(8)"
									v-if="canEditCountriesTraveled===false">
									<image src="/pagesB/static/esl/edit.png" mode="aspectFill"></image>
								</view>
								<view class="edit-icon" @click="countriesTraveledConfirm"
									v-if="canEditCountriesTraveled">
									<image src="/pagesB/static/esl/confirm-icon.png" mode="aspectFill"></image>
								</view>
							</view>
							<!-- 旅行过的国家展示tags -->
							<view class="countries-travel-b" v-if="canEditCountriesTraveled===false">
								<view class="countries-travel-item" v-for="(country,i) in countriesTraveledList"
									:key="i">
									<block v-if="languageValue=='en-US'">{{country.object_en}}</block>
									<block v-if="languageValue=='zh-CN'">{{country.object_cn}}</block>
								</view>
							</view>
							<!-- 旅行过的国家编辑tags -->
							<view class="jobs-tags-container" v-if="canEditCountriesTraveled">
								<view class="jobs-tags">
									<view class="jobs-tags-item"
										...
										v-for="(item,index) in editCountriesTraveledList" :key="item.id"
								...
								<view class="jobs-tags-item" v-if="addCountriesTraveledStatus==false"
									@click="addCountriesTraveledStatus=true">Add+</view>
								<view class="jobs-tags-add">
									<view class="jobs-tags-item-add" v-if="addCountriesTraveledStatus">
										<input type="text" v-model="ownCountriesTraveledValue"
											placeholder="Add countries traveled">
										<view class="jobs-tags-item-add-button">
											<button type="default" v-if="ownCountriesTraveledValue.length>0"
												@click="addOwnCountriesTraveled">Confirm</button>
											<button type="default" v-if="ownCountriesTraveledValue.length==0"
												@click="addCountriesTraveledStatus=false">Cancel</button>
										</view>
									</view>
								</view>
							</view>
						...
						<view class="countries-travel">
							<view class="countries-travel-t">
								<view class="countries-travel-title countries-travel-fontsize">
									{{i18n.profilecountrieslived}}</view>
							...
							<view class="jobs-tags-container" v-if="canEditCountriesLived">
								<view class="jobs-tags">
									<view class="jobs-tags-item"
										...
										v-for="(item,index) in editCountriesLivedList" :key="item.id"
								...
								<view class="jobs-tags-item" v-if="addCountriesLivedStatus==false"
									@click="addCountriesLivedStatus=true">Add+</view>
								<view class="jobs-tags-add">
									<view class="jobs-tags-item-add" v-if="addCountriesLivedStatus">
										<input type="text" v-model="ownCountriesLivedValue"
											placeholder="Add countries lived">
										<view class="jobs-tags-item-add-button">
											<button type="default" v-if="ownCountriesLivedValue.length>0"
												@click="addOwnCountriesLived">Confirm</button>
											<button type="default" v-if="ownCountriesLivedValue.length==0"
												@click="addCountriesLivedStatus=false">Cancel</button>
										</view>
									</view>
								</view>
							</view>
						...
						<view class="work-exp">
							<view class="work-exp-t">
								<view class="work-exp-title">{{i18n.profileworkexperience}}<text
										class="error-star">*</text></view>
							...
							<view class="work-exp-b">
								<view v-for="(work,i) in workInfo" :key="i">
									<view class="work-exp-b-item" v-if="i<=workExpNum">
										...
										<view class="work-exp-item-2">
											...
											<view class="work-exp-date">{{work.work_time_from | date('mm/yyyy')}} -
												{{work.work_time_to | date('mm/yyyy')}}</view>
								...
								<view class="show-more" v-if="showMoreWorkExpStatus" @click="showMoreWorkExp">Show more
								</view>
							</view>
						</view>
					</view>
					...
					<view class="subject">
						...
						<view class="jobs-tags-container" v-if="canEditHobby">
							<view class="jobs-tags">
								<block>
									<view class="jobs-tags-item"
										:class=" selectHobbyInfoList.findIndex((element)=>element===item) == -1 ? '' : 'tags-active' "
							...
							<view class="jobs-tags-item" v-if="addHobbyInfoStatus==false"
								@click="addHobbyInfoStatus=true">Add+</view>
							<view class="jobs-tags-add">
								<view class="jobs-tags-item-add" v-if="addHobbyInfoStatus">
									<input type="text" v-model="ownHobbyInfoValue" placeholder="Add Hobbies">
									<view class="jobs-tags-item-add-button">
										<button type="default" v-if="ownHobbyInfoValue.length>0"
											@click="addOwnHobby()">Confirm</button>
										<button type="default" v-if="ownHobbyInfoValue.length==0"
											@click="addHobbyInfoStatus=false">Cancel</button>
									</view>
								</view>
							</view>
						</view>
				...
				<view class="profile-media" v-if="current === 1">
					<view class="profile-detail-relative ">
						<view class="profile-detail-item-title">{{i18n.profileprofilephoto}}<text
								class="error-star">*</text></view>
						<view class="profile-detail-item-box" v-if="educatorInfo.profile_photo">
							<view class="profile-photo-1">
								<image @click="previewYourImage(educatorInfo.profile_photo)"
									:src="educatorInfo.profile_photo" mode="aspectFill"></image>
							</view>
						</view>

						<view class="profile-edit-button" @click="turnEditProfilePhoto(1,educatorInfo.profile_photo)">
							<!-- {{i18n.profileedit}} -->
							<image src="/pagesB/static/esl/upload.png" mode="aspectFit"></image>
						</view>
					</view>
					<view class="profile-detail-relative ">
						...
						<view class="profile-detail-item-box profile-header-photo" v-if="educatorInfo.header_photo">
							<view class="profile-header-photo-1">
								<image @click="previewYourImage(educatorInfo.header_photo)"
									:src="educatorInfo.header_photo" mode="aspectFill"></image>
							</view>
						</view>
					...
					<view class="profile-detail-relative ">
						<view class="profile-detail-item-title">{{i18n.profileintrovideo}}<text
								class="error-star">*</text></view>
						<view class="profile-detail-item-box" v-if="educatorInfo.video_url">
							<view class="profile-intro-video">
								<video id="myVideo" :muted="false" preload="metadata" @loadedmetadata="loadedMetaData"
									x5-video-player-type="h5-page" :src="educatorInfo.video_url"
				...
				<view class="preferences" v-if="current===2">
						...
						<view class="jobs-tags-container" v-if="canEditSubject">
							<view class="jobs-tags">
								<view class="jobs-tags-item"
									...
									v-for="(item,index) in editSubjectList" :key="item.id"
							...
							<view class="jobs-tags-item" v-if="addSubjectStatus==false" @click="addSubjectStatus=true">
								Add+</view>
							<view class="jobs-tags-add">
					...
					<view class="location">
						<view class="location-t">
							<view class="location-title">{{i18n.profilelocation}}<text class="error-star">*</text>
							</view>
						...
						<view class="jobs-tags-container" v-if="canEditLocation">
							<view class="jobs-tags">
								<view class="jobs-tags-item"
									...
									v-for="(item,index) in editLocationList" :key="item.id"
							...
							<view class="jobs-tags-item" v-if="addLocationStatus==false"
								@click="addLocationStatus=true">Add+</view>
							<view class="jobs-tags-add">
								<view class="jobs-tags-item-add" v-if="addLocationStatus">
									<input type="text" v-model="ownLocationValue" placeholder="Add location">
									<view class="jobs-tags-item-add-button">
										<button type="default" v-if="ownLocationValue.length>0"
											@click="addOwnLocation">Confirm</button>
										<button type="default" v-if="ownLocationValue.length==0"
											@click="addLocationStatus=false">Cancel</button>
									</view>
								</view>
							</view>
						</view>
						...
						<view class="jobs-tags-container" v-if="canEditJobType">
							<view class="jobs-tags">
								<view class="jobs-tags-item"
									...
									v-for="(item,index) in editJobTypeList" :key="item.id"
							...
							<view class="jobs-tags-add">
								<view class="jobs-tags-item-add" v-if="addJobTypeStatus">
									...
									<view class="jobs-tags-item-add-button">
										<button type="default" v-if="ownJobTypeValue.length>0"
											@click="addOwnJobType">Confirm</button>
										<button type="default" v-if="ownJobTypeValue.length==0"
											@click="addJobTypeStatus=false">Cancel</button>
									</view>
								</view>
							</view>
						</view>
						...
						<view class="jobs-tags-container" v-if="canEditAgeToTeach">
							<view class="jobs-tags">
								<view class="jobs-tags-item"
									...
									v-for="(item,index) in editAgeToTeachList" :key="item.id"
						...
						<view class="jobs-tags-container" v-if="canEditBenefits">
							<view class="jobs-tags">
								<view class="jobs-tags-item"
									...
									v-for="(item,index) in editBenefitsList" :key="item.id"
							...
							<view class="jobs-tags-item" v-if="addBenefitsStatus==false"
								@click="addBenefitsStatus=true">Add+</view>
							<view class="jobs-tags-add">
	...
	export default {
		data() {
			...
			return {
				...
				chartData: {},
				percent: '',
				items: [this.$t('index').educatorprofileitemsone, this.$t('index').educatorprofileitemstwo, this.$t(
					'index').educatorprofileitemsthree],
				current: 0,
		...
		methods: {
			...
			turnRateSkillsList(type) {
				uni.navigateTo({
					url: '/pagesB/me/profile/rateSkillsList?type=' + type
				})
			},
			turnIndexList(type) {
				let data = {
			...
			turnSearchTags(type) {
				uni.navigateTo({
					url: '/pagesB/me/profile/searchTags?type=' + type
				})
			},
			turnWorkExperience() {
				uni.navigateTo({
			...
			turnEditProfilePhoto(type, source) {
				...
				uni.navigateTo({
					url: '/pagesB/me/profile/photo?type=' + type + '&source=' + a
				})
			},
			turnEditProfilePhotoMulti(type, source) {
				// console.log(source);
				uni.navigateTo({
					url: '/pagesB/me/profile/photo?type=' + type,
					success() {
			...
			turnEditProfileVideo(type, source) {
				uni.navigateTo({
					url: '/pagesB/me/profile/video?type=' + type + '&source=' + source
				})
			},
			updateEduProfile() {
				let data = {
			...
			getBasicInfo() {
				...
				profile.getBasicInfo(data).then(res => {
					...
					if (res.code == 200) {
						...
						let share_data = {
							title: res.message.educator_info.first_name + res.message.educator_info
							.last_name, // 分享标题
		...
		onShareAppMessage: function() {
			...
			return {
				...
				path: '/pagesB/me/educator/share?id=' + uid
			}
		},
