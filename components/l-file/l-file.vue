		...
		methods: {
			...
			*/
					...
					success: ({
						...
						return uni.uploadFile({
							...
							success: (res) => {
								...
								return this.errorHandler('File upload failed', this.upErr);
							},
							fail: () => this.errorHandler('File upload failed', this.upErr)
						});
					},
					fail: (err) => this.errorHandler(err, this.upErr)
				})
			},
			hChooseFile({
				url,
			...
			hRequest({
				...
				let createUpload = (file, url, key = 'job_file', header = {}, data = {}) => {
			...
			*/
			upload(param = {}) {
				...
				if (this.loading) {
					this.toast('There is still a file in processing, please wait..');
					return;
				}
							...
							if (statusCode === 200) {
								if (type == 'local') {
									...
									uni.saveFile({
										...
										fail: () => this.errorHandler('download failed', reject)
									});
									// #endif
									// #ifdef H5
