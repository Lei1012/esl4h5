			...
			<view class="huakuaiBox">
				...
				<movable-area :animation="true">
					<movable-view :x="x" direction="horizontal" @change="onMove">
						<uni-icons type="forward" color="#ffffff"></uni-icons>
						<view class="pintukuai" :style="{top:top+'px'}">
							<image :src="imgUrl" :style="{top:-lefttop+'px',left:-left+'px'}"></image>
						</view>
					</movable-view>
				</movable-area>
				<view class="huadao">{{i18n.h5swipervertification}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		data() {
			return {
				imgUrl: 'https://osstest.esl-passport.cn/phpDiy4F420210408142917Nt.jpg',
				x: 0, //初始距离
				oldx: 0, //移动的距离
				img: '0', //显示哪张图片
				left: '', //随机拼图的最终X轴距离
